...
int main()
{
	...
	while (true)
	{
		...
		if (!strncmp(quitData, "quit", sizeof("quit")))
		{
			...
			else cout << "> active threads are remained : " << Socket::GetClientNumber() - 1 << " threads" << endl;
		}
	}
...
void ReceiveData(SOCKET clientSocket, int number)
{
	while (true)
	{
		...
		while (true)
		{
			if (::send(clientSocket, recvData, sizeof(recvData), 0) == SOCKET_ERROR)
			{
				if (::WSAGetLastError() == WSAEWOULDBLOCK) continue;
				HandleError("Send");
				break;
			}
			cout << "> echoed: " << recvData << " by Thread-" << number + 2 << endl;
			break;
		}
	}
